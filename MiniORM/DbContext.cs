...
namespace MiniORM
{
    public class DbContext
    {
        ...
        public void SaveChanges()
        {
            ...
            foreach (IEnumerable<object> dbSet in dbSets)
            {
                var invalidEntities = dbSet
                    .Where(x => !IsObjectValid(x)).ToArray();
        ...
        private void Persist<TEntity> (DbSet<TEntity> dbSet) where TEntity : class,new()
        {
            ...
            var modifiedEntitites = dbSet.ChangeTracker.GetModifiedEntities(dbSet).ToArray();
            if (!modifiedEntitites.Any())
            {
                _connection.UpdateEntities(modifiedEntitites,tableName,columns);
            }
            ...
            where TCollection : class, new()
        {
            ...
            var primaryKeys = collectionType.GetProperties().Where(pi => pi.HasAttribute<KeyAttribute>())
                .ToArray();
            ...
            where TEntity : class
        {
            ...
            TEntity[] fetchedRows = _connection.FetchResultSet<TEntity>(tableName, columns).ToArray();
        ...
        private string[] GetEntityColumnNames(Type table)
        {
            ...
            IEnumerable<string> dbColumns =_connection.FetchColumnNames(tableName);
        ...
        {
            foreach (var dbSetProp in _dbSetProperties)
            {
                var dbSetType = dbSetProp.Key;
                var mapRelationsGenetic = typeof(DbContext)
                    .GetMethod("MapRelations", BindingFlags.Instance | BindingFlags.NonPublic)
                    .MakeGenericMethod(dbSetType);
                var dbSet = dbSetProp.Value.GetValue(this);
                mapRelationsGenetic.Invoke(this, new[] { dbSet });
            }
        }
        ...
        private Dictionary<Type, PropertyInfo>? DiscoverDbSets()
        {
            var dbSets = GetType().GetProperties()
                .Where(pi => pi.PropertyType.GetGenericTypeDefinition() == typeof(DbSet<>)).ToDictionary(pi => pi.PropertyType.GetGenericArguments().First(), pi => pi);
            return dbSets;
        }
